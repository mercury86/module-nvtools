...
function nv_write_lang_mod_admin( $mod, $lang, $arr_new_lang )
{
	...
	if( ! empty( $arr_new_lang ) )
	{
		...
		{
			if( substr( $sys_info['os'], 0, 3 ) != 'WIN' ) chmod( NV_ROOTDIR . '/modules/' . $mod . '/language/admin_' . $lang . '.php', 0777 );
		}
...
if( preg_match( '/^[a-zA-Z0-9\_\-]+$/', $modname ) and preg_match( '/^[a-zA-Z0-9\_\-]+$/', $tablename ) )
{
	...
	try
	{
		...
		if( ! empty( $array_views ) )
		{
			...
			$lang_mod_admin_en = nv_get_lang_mod_admin( $modname, 'en' );
			...
			if( substr( $sys_info['os'], 0, 3 ) != 'WIN' ) chmod( NV_ROOTDIR . "/modules/" . $modname . "/admin/" . $funname . ".php", 0777 );
			...
			if( substr( $sys_info['os'], 0, 3 ) != 'WIN' ) chmod( NV_ROOTDIR . "/themes/admin_default/modules/" . $modname . "/" . $funname . ".tpl", 0777 );
